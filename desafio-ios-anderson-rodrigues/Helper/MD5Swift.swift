...
public func digestMD5(_ bytes: [Byte]) -> Digest {
    // Initialization
    let s: [Word] = [
    ...
    for chunk in 0..<totalChunks {
        ...
        let chunk: [Byte] = Array(message[index..<index+chunkLengthInBytes]) // 512-bit/64-byte chunk from var to let
